...
class TestFlaskApp(unittest.TestCase):
    ...
    def test_get_route_miles_post_valid_routes(self):
        ...
        response = self.app.post('/api/v1.0/get_route_miles', data=json.dumps(valid_data), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, 'application/json')
        data = json.loads(response.get_data(as_text=True))
    ...
    def test_get_route_miles_post_invalid_routes(self):
        ...
        response = self.app.post('/api/v1.0/get_route_miles', data=json.dumps(invalid_data), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content_type, 'application/json')
        data = json.loads(response.get_data(as_text=True))
        self.assertIn('1', data)
        ...
        self.assertIn('Miles', data['1'])
        self.assertIn('Miles', data['2'])
        # Depending on your logic, you might assert that the miles are 0 or that a specific error is returned
